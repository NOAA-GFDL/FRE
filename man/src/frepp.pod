...
=item B<-f>, B<--filelist> I<< <filelist> >>

List of files in F<$archiveDir/history> to be combined using B<mppnccombine>.
...
Still in BETA. Use month as base unit instead of year.  Use for short
...
complete.  I<Note>: The PP/AN nodes cannot set holds based on job
...
Duplicate postprocessing to a new unique directory.  The I<< <STRING>
>> value will be appended to the postprocessing output directory,
e.g. if 'frepp -u w' will produce F<.../pp_2>.
