# -*- Makefile -*-
# $Id: hsmget.mk,v 1.1.2.3 2011/01/28 17:02:55 afy Exp $
...
CPIO := cpio $(verbose) -C 524288 -d -i -I
...
$(PTMPROOT)/%.ok: $(ARCHROOT)/%
	@touch $@.LOCK
	-test -f $< && $(MKDIR) $(@D) && $(REMOTECP) `readlink -e $<` $(@D) || \
	 test -d $< && $(MKDIR) $(PTMPROOT)/$* && $(REMOTECP) `$(FIND) $< $(FINDFLAGS)` $(PTMPROOT)/$*
	-$(CHMOD) $(PTMPROOT)/$*
ifneq ($(check),)
	-test -f $< && cd $(@D) && $(SUM) $(*F) > $@ || \
...
$(PTMPROOT)/%.ok: $(ARCHROOT)/%.tar
	...
	cd $(PTMPROOT)/tmp/$(uuid) && $(TAR) `readlink -e $<` && mv -f $(PTMPROOT)/tmp/$(uuid)/* $(PTMPROOT)/$* && rm -rf $(PTMPROOT)/tmp/$(uuid)
	-$(CHMOD) $(PTMPROOT)/$*
ifneq ($(check),)
...
$(PTMPROOT)/%.ok: $(ARCHROOT)/%.cpio
	@touch $@.LOCK
	@$(MKDIR) $(PTMPROOT)/$* $(PTMPROOT)/tmp/$(uuid)
	cd $(PTMPROOT)/tmp/$(uuid) && $(CPIO) `readlink -e $<` && mv -f $(PTMPROOT)/tmp/$(uuid)/* $(PTMPROOT)/$* && rm -rf $(PTMPROOT)/tmp/$(uuid)
	-$(CHMOD) $(PTMPROOT)/$*
ifneq ($(check),)
	cd $(PTMPROOT)/$* && $(SUM) `$(FIND) . -type f` > $@
else
