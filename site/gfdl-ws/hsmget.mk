# -*- Makefile -*-
# $Id: hsmget.mk,v 1.1.2.2 2011/04/01 17:58:37 afy Exp $
...
$(PTMPROOT)/%.ok: $(ARCHROOT)/%.tar
	...
	$(LOCALCP) $< $(PTMPROOT)/tmp/$(uuid)
	cd $(PTMPROOT)/tmp/$(uuid) && $(TAR) $(<F) && rm -f $(<F) && mv -f $(PTMPROOT)/tmp/$(uuid)/* $(PTMPROOT)/$* && rm -rf $(PTMPROOT)/tmp/$(uuid)
ifneq ($(check),)
...
$(PTMPROOT)/%.ok: $(ARCHROOT)/%.cpio
	...
	@$(MKDIR) $(PTMPROOT)/$* $(PTMPROOT)/tmp/$(uuid)
	$(LOCALCP) $< $(PTMPROOT)/tmp/$(uuid)
	cd $(PTMPROOT)/tmp/$(uuid) && $(CPIO) $(<F) && rm -f $(<F) && mv -f $(PTMPROOT)/tmp/$(uuid)/* $(PTMPROOT)/$* && rm -rf $(PTMPROOT)/tmp/$(uuid)
# 	test -d $@ && cd $@ && $(CPIO) $< || \
