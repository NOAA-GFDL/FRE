...
# ------ implement conditional parsing based on the hostname
{
  ...
  if (open my $handle, '<', $f)
  {
	...
	{
	  FREMsg::out($v, FREMsg::FATAL, "File '$f' line $.: nested '#if' isn't allowed");
	  $ok = 0;
	  last;
	}
      }
	...
	{
	  FREMsg::out($v, FREMsg::FATAL, "File '$f' line $.: '#elsif' must follow '#if' or '#elsif'");
	  $ok = 0;
	  last;
	}
      }
	...
	{
	  FREMsg::out($v, FREMsg::FATAL, "File '$f' line $.: '#else' must follow '#if' or '#elsif'");
	  $ok = 0;
	  last;
	}
      }
	...
	{
	  FREMsg::out($v, FREMsg::FATAL, "File '$f' line $.: '#endif' must follow '#if' or '#elsif' or '#else'");
	  $ok = 0;
	  last;
	}
      }	
	...
	if (FREExternalProperties::propertyNameCheck($key))
	{
	  ...
	  {
            FREMsg::out($v, FREMsg::FATAL, "File '$f' line $.: property name '$key' is already defined");
	    $ok = 0;
	    last;
	  }
	}
	...
	{
          FREMsg::out($v, FREMsg::FATAL, "File '$f' line $.: property name '$key' is not an identifier");
	  $ok = 0;
	  last;
	}
      }
      ...
      {
        FREMsg::out($v, FREMsg::FATAL, "File '$f' line $.: invalid property syntax '$line'");
	$ok = 0;
	last;
      }
    }
