...
#----Now loop through each analysis under the current TS or AV node
   foreach my $ananode ( $ts_av_Node->findnodes('analysis')) {
     ...
     foreach my $addexptNode ( $ananode->findnodes('addexpt')) {
       ...
       (my $first_ref,my $last_ref) = availablechunks($asrcdir_addexpt,$component,$clnumber,$freq);
       ...
       $arrayofExptsH[$iExpt]{hist_dir} = "$archivedir/$arrayofExptsH[$iExpt]{exptname}/history";
     ...
     if (! $aScript) { print STDERR "ANALYSIS: analysis template script not specified, skipped\n";next;}
     ...
     if ($arrayofExptsH[0]{specify1year}) {
        ...
        filltemplate(\@arrayofExptsH,cleanpath($figureDir),$aScript,\@aargu,cleanpath($aScriptout),$iExpt,cleanpath($workdir),$mode,cleanpath($asrcfile),$opt_s,$opt_v,cleanpath($frexml),cleanpath($stdoutdir));
     #
     } else {
       ...
       if ($cumulative eq "yes" or $cumulative eq "YES") {
        ...
        #----#---- fill the variables in the template
         ...
         if ($tsORav eq "timeAverage") { 
           ...
           filltemplate(\@arrayofExptsH,cleanpath($figureDir),$aScript,\@aargu,cleanpath($aScriptout),$iExpt,cleanpath($workdir),$mode,"",$opt_s,$opt_v,cleanpath($frexml),cleanpath($stdoutdir)); 
         }
...
sub availablechunks {
   ...
   if (@existingall < 1) {
      ...
      if (@cpios < 1) {
         ...
         print STDERR "ANALYSIS: Try extracting the necessary cpio files in this directory first.\n";
      }
      return;
   }
...
# fill the template with the passing variables
    my ($arrayofExptsH_ref,$figureDir,$aScript,$aargu,$aScriptout,$iExpt,$workdir,$mode,$asrcfile,$opt_s,$opt_v,$frexml,$stdoutdir) = @_; 
...
#    my ($aScriptout,$aScript, $workdir, $mode, $momGrid,$gridspec,$staticfile,$asrcdir,$asrcfile,$expt,$figureDir,$astartYear,$aendYear,$databegyr,$dataendyr,$clnumber,$specify1year,$archivedir) = @_; 
    ...
    if ($iExpt >= 1) {
     for my $i (1 .. $iExpt) {
       ...
       $tmpsch =~ s/set MODEL_start_yr_$ii\s*$/set MODEL_start_yr_$ii = $arrayofExptsH_ref->[0]->{MODEL_start_yr}/m;
       $tmpsch =~ s/set mom_version_$ii\s*$/set mom_version_$ii = $arrayofExptsH_ref->[$i]->{momGrid}/m;
       $tmpsch =~ s/set gridspecfile_$ii\s*$/set gridspecfile_$ii = $arrayofExptsH_ref->[$i]->{gridspec}/m;
       $tmpsch =~ s/set staticfile_$ii\s*$/set staticfile_$ii = $arrayofExptsH_ref->[$i]->{staticfile}/m;
       $tmpsch =~ s/set in_data_dir_$ii\s*$/set in_data_dir_$ii = $arrayofExptsH_ref->[$i]->{asrcdir}/m;
       ...
       $tmpsch =~ s/set descriptor_$ii\s*$/set descriptor_$ii = $arrayofExptsH_ref->[$i]->{exptname}/m;
       ...
       $tmpsch =~ s/set databegyr_$ii\s*$/set databegyr_$ii = $arrayofExptsH_ref->[$i]->{databegyr}/m;
       $tmpsch =~ s/set dataendyr_$ii\s*$/set dataendyr_$ii = $arrayofExptsH_ref->[$i]->{dataendyr}/m;
       $tmpsch =~ s/set datachunk_$ii\s*$/set datachunk_$ii = $arrayofExptsH_ref->[$i]->{clnumber}/m;
       # specify_yr is a particular year, set by user, for daily data
       $tmpsch =~ s/set specify_yr_$ii\s*$/set specify_yr_$ii = $arrayofExptsH_ref->[0]->{specify1year}/m;
       $tmpsch =~ s/set hist_dir_$ii\s*$/set hist_dir_$ii = $arrayofExptsH_ref->[$i]->{hist_dir}/m;
...
# $dataendyr (output): data end date. High edge of $aendYear
     ...
     if ($astartYear > $aendYear ) {
         print STDERR "ANALYSIS: Date setting ERROR: begin year $astartYear larger than ending year $aendYear\n";
         $flag = "bad";
         return $flag;}
...
sub checkExptExists {
   ...
   if ( $nodecount gt 1 ) {
      print STDERR "WARNING: Multiple experiments called $e were found in $frexml.\nWARNING: Using first instance.\n";
   }
   return 1;
}
