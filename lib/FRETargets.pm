...
# ------ return standard representation of the target list (as a string) and optional error message
{
  ...
  if ($l)
  {
        ...
        if (scalar(grep($_ eq $targetFollower, @targets)) > 0)
	{
      ...
      {
        if (scalar(grep($_ eq $target, TARGET_STARTERS)) == 0 and scalar(grep($_ eq $target, TARGET_FOLLOWERS)) == 0)
	{
	...
	return ('', "Targets '$userTargets' are contradictory - not more than one user target is allowed");
      }
    }
    else
    {
      my $targetStarters = join(',', @targetStarters);
      return ('', "Targets '$targetStarters' are contradictory - only one of them is allowed");
    } 
  }
  else
  {
