...
system "frerun -x $xml -p $platform $exp -o -r nothreads";
isnt($?, 0, 'ranks/nothreads case gives error');

system "frerun -x $xml -p $platform $exp -o -r noranks";
isnt($?, 0, 'noranks/threads case gives error');

system "frerun -x $xml -p $platform $exp -o -r noranks-nothreads";
